...
class Prompt:
    ...
    for version management and variable substitution.
    """
...
class PromptManager:
    """
    Manages prompts in AWS Bedrock.
    ...
    def _get_prompt_content(self, prompt_id: str) -> str | None:
        ...
        try:
            ...
            if variants:
                # Get the first variant's template text
    ...
    def _create_prompt(self, name: str, prompt_text: str, description: str) -> dict:
        ...
        try:
            input_variables = self.extract_variables(prompt_text)
            text_config = {'text': prompt_text}
            if input_variables:
                text_config['inputVariables'] = input_variables
    ...
    def _create_new_version(self, prompt_id: str, prompt_text: str, description: str) -> dict:
        """Update prompt draft and create a new version."""
        input_variables = self.extract_variables(prompt_text)
        text_config = {'text': prompt_text}
        if input_variables:
            text_config['inputVariables'] = input_variables
