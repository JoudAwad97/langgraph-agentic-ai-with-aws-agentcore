...
def get_observability_env_vars(
    agent_name: str = "restaurant-finder-agent",
    runtime_id: str = "",
    log_group: str = "",
    region: str = "us-east-2",
) -> dict:
    """
    Generate environment variables for OpenTelemetry configuration.

    These should be set before running the agent with opentelemetry-instrument.
    For agents hosted on AgentCore Runtime, these are set automatically.
    For non-runtime hosted agents, these need to be set manually.

    Args:
        agent_name: Name of the agent service
        runtime_id: AgentCore Runtime ID (if applicable)
    ...
    """
    log_group = log_group or f"/aws/bedrock-agentcore/runtimes/{agent_name}"
...
def setup_cloudwatch_observability(
    ...
    """
    Set up CloudWatch delivery sources for observability.

    This creates the necessary CloudWatch log groups and delivery
    configurations to receive traces and logs from the agent.

    Note: This is typically only needed for non-runtime hosted agents.
    AgentCore Runtime agents have this set up automatically.

    Args:
        resource_arn: ARN of the AgentCore resource (memory, gateway, etc.)
        resource_id: ID of the resource
        account_id: AWS account ID
        region: AWS region

    Returns:
        Dictionary with delivery IDs for logs and traces
    """
    import boto3

    logs_client = boto3.client("logs", region_name=region)

    # Create log group for vended log delivery
    ...
    try:
        ...
        result["logs_source_name"] = logs_source["deliverySource"]["name"]
        ...
        result["traces_source_name"] = traces_source["deliverySource"]["name"]

        # Create delivery destination for logs
        ...
        result["logs_destination_arn"] = logs_dest["deliveryDestination"]["arn"]
        ...
        result["traces_destination_arn"] = traces_dest["deliveryDestination"]["arn"]

        # Create deliveries
        logs_delivery = logs_client.create_delivery(
            deliverySourceName=result["logs_source_name"],
            deliveryDestinationArn=result["logs_destination_arn"],
        )
        result["logs_delivery_id"] = logs_delivery["delivery"]["id"]

        traces_delivery = logs_client.create_delivery(
            deliverySourceName=result["traces_source_name"],
            deliveryDestinationArn=result["traces_destination_arn"],
        )
        result["traces_delivery_id"] = traces_delivery["delivery"]["id"]

        logger.info(f"CloudWatch observability configured for {resource_id}")

    except Exception as e:
        logger.warning(f"Could not set up CloudWatch delivery: {e}")
...
def get_observability_manager() -> ObservabilityManager:
    ...
    if _observability_manager is None:
        ...
        service_name = os.getenv("OTEL_SERVICE_NAME", "restaurant-finder-agent")
